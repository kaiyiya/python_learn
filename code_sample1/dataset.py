        ...
        if self.augment and len(self.aug_transforms) > 0:
            ...
            for name in selected:
                if name == 'hflip':
                    ...
                    img_ops.append(transforms.RandomAffine(degrees=0, translate=(0.05, 0.05), scale=(0.9, 1.1), shear=5,
                    ...
                    try:
                        img_ops.append(transforms.ElasticTransform(alpha=10.0, sigma=3.0,
                                                                   interpolation=InterpolationMode.BILINEAR))
                        mask_ops.append(
                            transforms.ElasticTransform(alpha=10.0, sigma=3.0, interpolation=InterpolationMode.NEAREST))
                    except TypeError:
