...
'''
基于指纹特征的指纹识别，一直以来都受到学术界和工业界的高度关注。
最近，Cappelli在WSB2021（Winter School on Biometrics 2021）
上做了主题为
《使用 OpenCV 和 Python 进行指纹识别
（Hands on Fingerprint Recognition with OpenCV and Python）》
的讲演，介绍了指纹识别的最新研究成果，
并使用OpenCV和Python进行了实现。
在香港浸会大学的WSB2021内容页上可以下载该讲稿及程序源代码（ipynb格式）。
其网址为：
https://www.comp.hkbu.edu.hk/wsb2021/lecturer_details.php?lect_id=2
'''